...
<!--
  RBCCM Two-Up Cards :: video-callouts (single-skin build)
  ============================================================
  Standalone skin for TeamSite's Skin dropdown. Renders ONLY the
  video-callouts variant ("Find conviction" parity): light
  section, each card leads with an image + play button, then
  title + optional subtitle + body + CTA. The play button and
  the CTA open the video modal (rbccm-two-up-cards.js).

  Companion skin: rbccm-two-up-cards (double-dash) features.xsl
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/">
    ...
    <section>
      <xsl:attribute name="class">rbccm-two-up-cards rbccm-two-up-cards--video-callouts</xsl:attribute>
    ...
    <!-- Video modal runtime (play button + "Watch the video" link). -->
    <xsl:if test="$JS_PATH != ''">
      <script>
