                          ...
                          <xsl:choose>
                            ...
                            <xsl:otherwise>
                              <xsl:choose>
                                ...
                                <xsl:when test="contains($ARTICLE_ID, 'episode/data/energy')">/en/insights/energy-transition/episode/<xsl:value-of select="$ARTICLE_TEMPLATE_TITLE" /></xsl:when>
                                <xsl:when test="contains($ARTICLE_ID, 'episode/data/ma')">/en/gib/ma-inflection-points/episode/<xsl:value-of select="$ARTICLE_TEMPLATE_TITLE" /></xsl:when>
                                <xsl:when test="contains($ARTICLE_ID, 'article/podcast')">
                                  <xsl:choose>
                                    <xsl:when test="contains(upper-case($ARTICLE_SERIES), upper-case('esg'))">/en/insights/markets-in-motion/podcast.page?dcr=<xsl:value-of select="$ARTICLE_ID" /></xsl:when>
                                    <xsl:when test="contains(upper-case($ARTICLE_SERIES), upper-case('markets'))">/en/insights/markets-in-motion/podcast.page?dcr=<xsl:value-of select="$ARTICLE_ID" /></xsl:when>
                                    <xsl:when test="contains(upper-case($ARTICLE_SERIES), upper-case('industries'))">/en/insights/industries-in-motion/podcast.page?dcr=<xsl:value-of select="$ARTICLE_ID" /></xsl:when>
                                    <xsl:when test="contains(upper-case($ARTICLE_SERIES), upper-case('real'))">/en/insights/the-real-pulse/podcast.page?dcr=<xsl:value-of select="$ARTICLE_ID" /></xsl:when>
                          ...
                          <div class="row">
                            <div class="col-md-4">
                              <div class="img-stretch inline">
                                <xsl:attribute name="style">width: auto; background-image: url('<xsl:value-of select="$ARTICLE_THUMBNAIL" />');</xsl:attribute>
                            ...
                            <div class="col-md-8">
                              <div class="white-box-text">
                                ...
                                <p class="description" style="padding-bottom: 0px;">
                                  <xsl:choose>
                                    <xsl:when test="string-length($ARTICLE_DESCRIPTION) > $CHARACTER_LIMIT">
                                      <xsl:for-each select="tokenize(substring($ARTICLE_DESCRIPTION, 0, $CHARACTER_LIMIT), ' ')">
                                        <xsl:if test="position() != last()">
                                          <xsl:if test="position() != 1"><xsl:text> </xsl:text></xsl:if>
                                          <xsl:choose>
                                            <xsl:when test="position() = last()-1">
                                              <xsl:choose>
                                                <xsl:when test="substring(normalize-space(.), string-length(normalize-space(.)), string-length(normalize-space(.))) = ','">
                                                  <xsl:value-of select="substring(normalize-space(.), 1, string-length(normalize-space(.)) - 1)" disable-output-escaping="yes" />
                                                </xsl:when>
                                                <xsl:otherwise>
                                ...
                                <div class="external-button-group">
                                  <a href="javascript:void(0)" aria-label="Player">
                                  ...
                                  <a class="learn-more audio-play btn-close-player" target="_blank">
                                    <xsl:attribute name="style">
                                      <xsl:choose>
                                        <xsl:when test="$ARTICLE_PODCAST = ''">display: none</xsl:when>
                                    ...
                                    <xsl:if test="$ARTICLE_WATCHTIME != ''">
                                      <xsl:choose>
                                        <xsl:when test="$ARTICLE_TYPE = 'video'">
                                          <img src="{$WATCH_ICON}" alt="Watch Time"/><xsl:text> </xsl:text><xsl:value-of select="$ARTICLE_WATCHTIME"/>
                                        </xsl:when>
                                        <xsl:otherwise>
                                          <img src="{$LISTEN_ICON}" alt="Listen Time"/><xsl:text> </xsl:text><xsl:value-of select="$ARTICLE_WATCHTIME"/>
                                        </xsl:otherwise>
                                      </xsl:choose>
                                    </xsl:if>
                                    <xsl:if test="($ARTICLE_READTIME != '') and ($ARTICLE_WATCHTIME != '')">
                                      <span class="separator" role="presentation" style="display: inline-block;height: 15px;border-left: 1px #000000 solid;opacity: .5;margin: 0 7px;"></span>
                                    </xsl:if>
                                    <xsl:if test="$ARTICLE_READTIME != ''">
                                      <img src="{$READ_ICON}" alt="Read Time"/><xsl:text> </xsl:text><xsl:value-of select="$ARTICLE_READTIME"/>
                                    </xsl:if>
                                  </p>
                                </div>
                              </div>
      ...
      <div class="container" style="display: none" data-bind="visible: show() &gt;= 0">
        <div class="row">
          ...
          <div data-bind="foreach:filteredItems">
            <div data-bind="visible: $index() &lt; $parent.show(), css: {{'col-md-12': ($index() &gt;= 0), 'col-sm-12': true}}">
                ...
                <div data-bind="css: {{'tile--white': ($index() >= 0)}}">
                  ...
                  <div data-bind="visible: $index() &gt;= 0">
                    <div class="col-md-4 ko-mobile">
                      <div class="img-stretch inline" data-bind="attr: {{style: 'width:auto; background-image: url(\''+thumbnail+'\')'}}">
                        <div class="tile-overlay"></div>
                      </div>
                    </div>
                    <div class="col-md-8">
                        ...
                        <div class="row">
                          ...
                          <div class="col-md-5 col-sm-12">
                            <div class="external-button-group">
                              <div class="story-podcast-playing" style="display: none;">
                                <button aria-label="Close Player" class="btn-close-player close-btn"><img src="/assets/rbccm/images/campaign/player-x.svg" alt="Close" /></button>
                                <iframe width="100%" height="200" scrolling="no" frameborder="no" allow="autoplay" data-bind="visible: podcast != '', attr:{{src: podcast}}"></iframe>
                              </div>
                            </div>
                          </div>
                        </div>
