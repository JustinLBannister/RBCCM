/* =========================================
   RBC CM -- Featured Conferences (RETRO / V1)
   Scoped to #rbccm-featured-conferences

   Tab logic only (no carousels). Differs from the V2 JS:
     - Default inner tab is '-ghc' (the first VISIBLE inner tab in V1),
       not '-overview'.
     - No speakers / insights slider initialization.
   Pair with featured-conferences-retro.html + .css.
   ========================================= */
(function () {
  ...
  function activateOuter(key) {
    ...
    // V1 = '-ghc' (Global Healthcare); the Overview/Speakers/Insights/-gep
    // tabs are hidden via CSS in the retro layout.
    setExclusiveInner(key + '-ghc');
  }
  ...
  window.addEventListener('resize', function () {
    ...
    rTimer = setTimeout(function () {
      ...
      // Both directions: open the first visible inner tab ('-ghc') of the
      // visible conference.
      setExclusiveInner(activeKey + '-ghc');
    }, 150);
  });
})();
