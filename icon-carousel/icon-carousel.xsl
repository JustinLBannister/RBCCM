...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/">
    ...
    <xsl:if test="$itemCount >= 4">
      ...
      <!-- Case-insensitive: a free-text Datum will get "Dark" sooner or later,
           and an exact match would silently fall back to light with no error. -->
      <xsl:variable name="colorScheme">
        <xsl:choose>
          <xsl:when test="translate(normalize-space(/Properties/Datum[@ID='ColorScheme']),
                          'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz') = 'dark'">dark</xsl:when>
          <xsl:otherwise>light</xsl:otherwise>
        </xsl:choose>
      </xsl:variable>

      <!-- Accepts center / centered, any casing — same reasoning as ItemAlignment. -->
      <xsl:variable name="headingAlignRaw" select="translate(normalize-space(/Properties/Datum[@ID='HeadingAlignment']),
      ...
      <!-- Item layout: 'inline' (icon beside label, per the Figma comp) or
           'centered' (icon above label, both centred).

           Accepts "center" AND "centered", any casing. This is deliberately
           forgiving: HeadingAlignment right above it takes "center", so an
           author will reasonably type "center" here too — and an exact-match
           test on "centered" would then silently do nothing, with no error to
           explain why. A free-text Datum has to tolerate the obvious spelling.

           Anything else falls back to inline. -->
      <xsl:variable name="itemAlignRaw"
        select="translate(normalize-space(/Properties/Datum[@ID='ItemAlignment']),
                          'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')"/>

      <xsl:variable name="itemAlignment">
        <xsl:choose>
          <xsl:when test="$itemAlignRaw = 'centered' or $itemAlignRaw = 'center'">centered</xsl:when>
