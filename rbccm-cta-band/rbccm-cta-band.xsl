...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <!--
    Skin: CTA Band — shared component (v2, Preset-driven).

    Renders one of three visual variants (Preset Datum):

      maas-mata   Big serif heading + yellow highlight +
                            1 yellow pill CTA. Own navy gradient.
                            MAAS+MATA "Talk with an expert." pattern.

      strategy-and-economics       Small serif heading (28px) + body copy
                            (both wrapped in .__intro for tight
                            rhythm) + 2 buttons (primary yellow +
                            secondary outlined ghost). Deep navy.
                            S&E "Log in to RBC insights" pattern.

      us-credentials        Big serif ramp like maas-mata,
                            but the heading uses flex-column at
                            desktop so the yellow highlight always
                            drops beneath the white lead. No eyebrow
                            slot. US Credentials "Ready to turn
                            insights into results?" pattern.

    Modifier: BgTransparent="yes" adds the bg-transparent modifier
    (.rbccm-cta-band + .rbccm-cta-band bg-transparent) which zeros
    the section background so a parent wrapper (e.g. MAAS+MATA's
    .rbccm-maas-mata__deep-band) can paint a continuous gradient
    through the band.
    ...
    live content from a JSON draft. Falls back to the XSL Datum
    value when no JSON is bound.
  -->
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="PRESET">
      <xsl:choose>
        ...
        <xsl:otherwise>maas-mata</xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    ...
    <!-- ═══ Stylesheet link ══════════════════════════════════
         ...
         ═══════════════════════════════════════════════════════ -->
    <link rel="stylesheet" href="/assets/rbccm/css/components/rbccm-cta-band.css"/>
