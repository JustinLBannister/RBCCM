...
Shared component. One TeamSite skin, three visual presets. All three render a centered stack (optional eyebrow → heading → body → button row) on a dark navy surface; the preset picks the typography ramp, the number of buttons, and whether heading + body sit inside a tight `.__intro` wrapper.
...
- `rbccm-cta-band.xsl` — TeamSite skin (Preset-driven). Renders from Datums, emits `data-json` attributes so the runtime binder can hydrate live content in preview mode.
- `rbccm-cta-band-properties.xml` — Datums (Preset picker + modifier + content + CTAs + section attrs).
...
Preset selection also owns the heading DOM shape:
...
|---|---|---|---|
| `Preset` | Enum | Yes | `maas-mata` (default) / `strategy-and-economics` / `us-credentials` |
| `BgTransparent` | Enum | Optional | `no` (default) / `yes`. Set to `yes` when nested in a wrapper that owns the gradient. |
...
| `Cta2Label` | String | Optional | Text on the secondary (outlined ghost) button. Only meaningful on `strategy-and-economics`; leave blank on the other presets. |
...
2. Register the skin in TeamSite from `rbccm-cta-band.xsl` + `rbccm-cta-band-properties.xml`.
...
5. Optional: add sample content to the CMS instance for each preset so authors have a template to start from.
