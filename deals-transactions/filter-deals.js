...
(function (window, document, $, ko) {
  ...
  /* ------------------------------------------------------------------ */
  /*  Helper: compute the correct scroll offset based on sticky state   */
  /* ------------------------------------------------------------------ */
  function getScrollOffset() {
    var bar = document.getElementById('yf-filter-bar');
    if (bar && bar.classList.contains('is-sticky')) {
      return STICKY_TOP + (bar.offsetHeight || 70) + 20;
    }
    return STICKY_TOP + 20;
  }

  /* ------------------------------------------------------------------ */
  /*  Helper: scroll to the right anchor after a filter / page change   */
  /* ------------------------------------------------------------------ */
  function scrollToFilteredContent() {
    ...
    if (bar.classList.contains('is-sticky')) {
      target = document.querySelector('.insights-stories.ko');
      if (target) {
        window.scrollTo({
      ...
      window.scrollTo({
        top: bar.getBoundingClientRect().top + window.scrollY - offset,
        behavior: 'smooth'
      });
    }
  }
    ...
    self.loadMore = function () {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          if (filterBar) {
            var offset = $(filterBar).offset().top - getScrollOffset();
    ...
    self.initFilterBar = function () {
      ...
      if (!document.getElementById('yf-filter-bar-styles')) {
        ...
        baseStyle.textContent = [
          ...
          '@media (max-width:1199px){#yf-filter-bar.is-sticky{padding:0 15px !important;}}',
          '@media (max-width:1199px){#yf-filter-bar.is-sticky > .is-sticky{padding:0 15px !important;}}',
          '@media (min-width:1200px){#yf-filter-bar.is-sticky{padding:0 15px !important;}}',
          '@media (min-width:1200px){#yf-filter-bar.is-sticky > .is-sticky{padding:0 15px !important;}}'
        ].join('');
      ...
      style.textContent = [
        ...
        '#yf-filter-bar .container.is-sticky{gap: 0 !important;}',
        '@media (max-width:1199px){#yf-filter-bar.is-sticky{padding:0 15px !important;}}',
      ...
      /* ---- FIX: spacer element to prevent content jump when sticky ---- */
      var spacer = document.createElement('div');
      spacer.id = 'yf-sticky-spacer';
      ...
      function makeSticky() {
        ...
        // Capture height BEFORE going fixed so spacer gets the right value
        var barHeight = filterBar.offsetHeight;

        isSticky = true;

        filterBar.classList.remove('container');
        filterBar.classList.add('is-sticky');       // FIX: add to filterBar itself
        inner.classList.add('is-sticky');
        ...
        // FIX: show spacer to prevent content from jumping behind sticky bar
        spacer.style.cssText = 'display:block;height:' + barHeight + 'px;';
      }
      ...
      function makeNormal() {
        ...
        filterBar.classList.add('container');
        filterBar.classList.remove('is-sticky');     // FIX: remove from filterBar itself
        inner.classList.remove('is-sticky');

        filterBar.style.cssText = 'background:#fff;border:1px solid #d3d3d3;margin-bottom:20px;padding:0;';

        // FIX: hide spacer when not sticky
